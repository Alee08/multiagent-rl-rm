...
class IntervalEventDetector:
    """Detect interval events on a one-dimensional continuous state."""

    def __init__(
        self,
        a_interval: Tuple[float, float] = (0.35, 0.45),
        b_interval: Tuple[float, float] = (0.55, 0.65),
    ):
    ...
    def detect_event(self, state):
        x = state["x"]
        if self.a_interval[0] <= x <= self.a_interval[1]:
            return "A"
        if self.b_interval[0] <= x <= self.b_interval[1]:
            return "B"
        return None


class ContinuousLineAgent:
...
class ContinuousLineSequence:
    """A small continuous-state NMRDP with a sequential A-then-B reward."""

    def __init__(
        self,
        *,
        seed: int,
        event_detector: Optional[IntervalEventDetector] = None,
        step_size: float = 0.025,
        noise_std: float = 0.005,
        horizon: int = 60,
    ):
        self.rng = np.random.default_rng(seed)
        self.event_detector = event_detector or IntervalEventDetector()
    ...
    def step(self, action):
        ...
        self.x = min(0.999999, max(0.0, self.x + direction * self.step_size + noise))
        event = self.event_detector.detect_event({"x": self.x})
        self.q_idx, reward, solved = self._rm_transition(self.q_idx, event)
        self.steps += 1
        truncated = self.steps >= self.horizon
        return {"x": self.x}, self.q_idx, reward, solved or truncated, solved
...
def make_encoder(
    *,
    ...
    detector = event_detector or IntervalEventDetector()
    return BucketStateEncoder(
        ContinuousLineAgent(detector),
        features=["x"],
        lows=[0.0],
        highs=[1.0],
        buckets=[buckets],
...
def run_bucket_qrmax_trial(
    *,
    ...
    for episode in range(train_episodes):
        ...
        if (
            first_perfect_eval_episode is None
            and (episode + 1) % eval_interval == 0
        ):
            eval_env = ContinuousLineSequence(
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Run the continuous-line Bucket QR-MAX sanity experiment."
    )
    parser.add_argument("--algorithm", choices=["QRMAX", "QL", "RMAX"], default="QRMAX")
    parser.add_argument("--seeds", default="1500:1520")
    ...
    print(
        "algorithm,event_aware,buckets,threshold,noise,seeds,"
        "success_mean,success_min,success_max,avg_len"
    )
    print(
        f"{summary['algorithm']},{summary['include_event_label']},"
        f"{args.buckets},{args.threshold},{args.noise_std},"
